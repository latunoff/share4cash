 ...
 * Connection:
 *
 * $db = new AjaxisDB(); // with default settings
 * 
 * $opts = array(
 *		'user'    => 'user',
 *		'pass'    => 'pass',
 ...
 *		'charset' => 'latin1'
 * );
 * $db = new AjaxisDB($opts); // with some of the default settings overwritten
 * 
...
class IsDB
{
	...
	public function exist($t){
		//echo $this->db_name;
		$tablesInDb = @mysql_query('SHOW TABLES FROM '.$this->db_name.' LIKE "'.$t.'"');
        if($tablesInDb)
        {
    ...
    public function insert()	//first argument is database name, second is associative array
	{	
		//ea($this);
		$args = func_get_args();
        array_unshift($args, "INSERT INTO ?n SET ?u");
        if(isset($args[2]['sort_id']) && strstr($args[2]['sort_id'],'?sort')){
            if(strstr($args[2]['sort_id'],'-'))$sort_depends = substr($args[2]['sort_id'], strpos($args[2]['sort_id'], '-')+1);
            $sort_max = $this->getMax($args[1],'sort_id',$sort_depends);
            $args[2]['sort_id'] = $sort_max+1;
        }
        //echo $this->prepareQuery($args);
	 ...
	 *	//first argument is database name,
			second is associative array,
			third - is value (number of id by default),
			4th - key field (id by default)
	 *
	 */
	public function update()	
	{	
		$args = func_get_args();
		array_unshift($args, "UPDATE ?n SET ?u WHERE `id`='?i'");	//insert query as 1st element of array
        //echo $this->prepareQuery($args);
		//die();
        return $this->rawQuery($this->prepareQuery($args));
	}
	
    public function getMax($table,$field='id',$depends=''){
        if($depends > '')$depends = " WHERE ".$depends;
        $query = $this->prepareQuery(array("SELECT MAX(`$field`) AS `mx` FROM ?n".$depends, $table));
        $var = $this->fetch($this->rawQuery($query));
        return $var["mx"];
    }
	
	public function getCount($table,$field='id',$depends=''){
        if($depends > '')$depends = " WHERE ".$depends;
        $query = $this->prepareQuery(array("SELECT COUNT(`$field`) AS `cnt` FROM ?n".$depends, $table));
        $var = $this->fetch($this->rawQuery($query));
        return $var["cnt"];
    }
	...
	{	
		$args = func_get_args();
		array_unshift($args, "DELETE FROM ?n WHERE `id`='?i'");	//insert query as 1st element of array
        //echo $this->prepareQuery($args);
		//die();
        return $this->rawQuery($this->prepareQuery($args));
	}
